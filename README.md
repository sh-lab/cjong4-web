...
選択、実対局の進行、合法手入力、再生・一時停止・次の停止地点までの進行、履歴の復元に対応しています。
...
5. 対局履歴のスライダーで任意の状態へ戻れます。過去から進行すると後続履歴は破棄されます。
...
履歴復元、UI snapshot JSONを公開しています。返却JSON文字列はライブラリ所有で、次回呼び出しまで有効です。
